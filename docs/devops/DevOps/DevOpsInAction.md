...
```docker
$ docker run --name artifactory-oss-6.18.1 -d -p 8083:8081 docker.bintray.io/jfrog/artifactory-oss:6.18.1
```
...
$ curl -u admin:AKCp8krAbJnpHRmCxRwQyh2t58cC6Hn6zURWKFwMaHGKvG7LP7FHRzLVuMBsEYdGaGBFLpYVW http://emon:8083/artifactory/libs-release-local/com/coding/devops/notebook-service/1.0/notebook-service-1.0.jar --output notebook-service-1.0.jar
```
...
```bash
$ docker run --name sonarqube -d -p 9000:9000 library/sonarqube:lts
```
...
```bash
# -1不是-l
$ ls -1 /usr/local/python3/bin/ansible*
...
```bash
# -b -K 表示提升权限执行，K是大写字母
$ ansible web -m command -a 'ss -ntl' -b -K
emon3 | CHANGED | rc=0 >>
...
# 如果指定目录存在，则不执行命令
$ ansible web -m command -a 'creates=/data/ ls'
emon2 | SUCCESS | rc=0 >>
skipped, since /data/ existsDid not run command since '/data/' exists
emon3 | CHANGED | rc=0 >>
bin
# 如果指定目录不存在，则不执行命令
$ ansible web -m command -a 'removes=/data/ ls'
emon2 | CHANGED | rc=0 >>
bin
emon3 | SUCCESS | rc=0 >>
...
```bash
$ ansible web -m shell -a 'cat /etc/passwd|grep "emon"'
emon3 | CHANGED | rc=0 >>
...
$ ansible web -m shell -a 'ss -ntl' -b -K
```
...
```bash
$ ansible web -m copy -a 'src=~/notebook-service-1.0.jar dest=~/'
```
...
# 查看一下
$ ansible web -m shell -a 'ls -l ~/|grep test'
emon3 | CHANGED | rc=0 >>
...
```bash
# 效果：bbb.jpg -> aaa.jpg 
$ ansible web -m file -a 'path=~/data/bbb.jpg force=yes src=aaa.jpg state=link'
```
...
```bash
$ ansible web -m fetch -a 'src=~/test.txt dest=~/data'
# 查看结果
$ tree data
data
├── emon2
...
```bash
$ ansible web -m cron -a 'name="ntp update every 5 min" minute=*/5 job="/usr/sbin/ntpdate 172.17.0.1 &> /dev/null"'
# 查看结果
...
$ ansible web -m cron -a 'name="ntp update every 5 min" minute=*/5 job="/usr/sbin/ntpdate 172.17.0.1 &> /dev/null" state=absent'
# 查看结果
$ ansible web -m shell -a 'crontab -l'
emon2 | CHANGED | rc=0 >>
...
```bash
# 查看目标机器是否有该包
$ ansible web -m shell -a 'yum list epel-release'
# 安装
$ ansible web -m yum -a 'name=epel-release state=present' -b -K
```
...
```bash
# 安装
$ ansible web -m yum -a 'name=htop state=present' -b -K
# 卸载
$ ansible web -m yum -a 'name=htop state=absent' -b -K
```
...
```bash
$ ansible web -m service -a 'name=docker state=started enabled=true' -b -K
```

- 关闭服务并取消自启动

```bash
$ ansible web -m service -a 'name=docker state=stopped enabled=false' -b -K
```
...
```bash
$ ansible web -m user -a 'name=keer uid=11111' -b -K
# 查看结果
...
```bash
$ ansible web -m user -a 'name=keer state=absent' -b -K
# 查看结果
$ ansible web -m shell -a 'cat /etc/passwd|grep keer'
emon2 | FAILED | rc=1 >>
...
```bash
$ ansible web -m group -a 'name=songuo gid=12222' -b -K
# 查看结果
...
```bash
$ ansible web -m group -a 'name=songuo state=absent' -b -K
# 查看结果
$ ansible web -m shell -a 'cat /etc/group|grep 12222'
emon3 | FAILED | rc=1 >>
non-zero return code
...
```bash
$ vim ansibledata/ping-playbook.yml
```
...
```bash
$ ansible-playbook ansibledata/ping-playbook.yml
# 命令行输出结果
PLAY [web] ****************************************************************************************************
...
```bash
$ vim ansibledata/loop-playbook.yml
```
...
```bash
$ vim ansibledata/import-playbook.yml
```
...
```bash
$ ansible-playbook ansibledata/import-playbook.yml 
```
