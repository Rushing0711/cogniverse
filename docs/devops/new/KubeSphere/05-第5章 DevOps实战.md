...
#### 11.1.2 标记 CI 节点（<span style="color:#32CD32;font-weight:bold;">推荐</span>）
...
#### 11.1.3 给 CI 节点添加污点（<span style="color:red;font-weight:bold;">不推荐</span>）
...
#### 11.2.2 podTemplate base（镜像：1.16G）
...
5. 再为 SonarQube 创建一个凭证 (**sonar-token**)，用于上述的阶段 3（代码分析）。凭证类型选择**访问令牌**，在**令牌**字段输入 SonarQube 令牌，请参阅[为新项目创建 SonarQube Token](/devops/new/KubeSphere/04-第4章%20KubeSphere扩展服务安装.html#_2-4-为新项目创建-sonarqube-token)。点击**确定**完成操作。<span style="color:red;font-weight:bold;">（本流程使用的是全局的SonarQube令牌，且已经配置到Jenkins）</span>

6. 创建完成后，您将在凭证页面看到已创建的凭证。

#### 12.1.4 步骤 2：创建项目

本示例中，流水线会将 [devops-maven-sample](https://github.com/Rushing0711/devops-maven-sample/tree/v4.1.0-sonarqube) 应用部署至一个项目。因此，需要创建一个普通项目（例如 **kubesphere-sample-dev**）和一个devops项目（例如**kubesphere-sample-devops**）。待流水线成功运行，将在该项目中自动创建该应用的部署和服务。
...
| 字符串   | APP_NAME            | `devops-maven-sample` | 应用名称。本示例使用 **devops-maven-sample**。 |
...
- **URL**：输入 GitHub 仓库地址 https://github.com/Rushing0711/devops-maven-sample.git 。注意，这里是示例地址，请使用自己的仓库地址。
- **凭证 ID**：本示例中无需输入凭证 ID。
...
5. （可选）再为 SonarQube 创建一个凭证 (**sonar-token**)，用于上述的阶段 3（代码分析）。凭证类型选择**访问令牌**，在**令牌**字段输入 SonarQube 令牌，请参阅[为新项目创建 SonarQube Token](/devops/new/KubeSphere/04-第4章%20KubeSphere扩展服务安装.html#_2-4-为新项目创建-sonarqube-token)。点击**确定**完成操作。<span style="color:red;font-weight:bold;">（本流程使用的是全局的SonarQube令牌，且已经配置到Jenkins）</span>

6. 您将在凭证页面看到已创建的凭证。

#### 12.2.4 步骤 2：在 GitHub 仓库中修改 Jenkinsfile

1. 登录 GitHub 并 Fork GitHub 仓库 [devops-maven-sample](https://github.com/Rushing0711/devops-maven-sample/tree/v4.1.0-sonarqube) 的所有分支至您的 GitHub 个人账户。
...
| 凭证ID          | 类型                                                         |
| --------------- | ------------------------------------------------------------ |
| harbor-id       | 用户名和密码（密码填写Harbor机器人令牌）<br />name: 	robot$ks-devops-harbor+robot-test<br/>secret:	kepdu1k58Jjg8mAVFDeBrNRSQzrPDO1x |
