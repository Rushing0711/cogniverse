...
| --------- | -------- | --------------- | ---- | ----- | ----------------- |
| k8s-node1 | Rocky9   | 192.168.200.116 | 4核  | >=16G | 控制节点/工作负载 |
| k8s-node2 | Rocky9   | 192.168.200.117 | 4核  | >=16G | 控制节点/工作负载 |
| k8s-node3 | Rocky9   | 192.168.200.118 | 4核  | >=16G | 控制节点/工作负载 |

### 1.2 系统安装（所有节点）

[系统安装](/devops/new/Linux/01-%E7%AC%AC1%E7%AB%A0%20%E7%B3%BB%E7%BB%9F%E5%AE%89%E8%A3%85.html)
...
$ sudo dnf update -y
# 安装依赖包
$ sudo dnf install -y socat conntrack ebtables ipset ipvsadm jq sysstat curl iptables libseccomp yum-utils
```

#### 1.3.2 关闭防火墙、重置iptables、关闭swap、关闭selinux和dnsmasq
...
# 设置iptables规则
$ sudo sh -c 'iptables -F && iptables -X && iptables -F -t nat && iptables -X -t nat && iptables -P FORWARD ACCEPT'

# 关闭swap
$ sudo swapoff -a
# 去掉swap开机启动
$ sudo sed -i '/swap/s/^\(.*\)$/#\1/g' /etc/fstab
# 对于标准分区，创建了 swap 分区，还需要检查：
$ sudo systemctl list-unit-files | grep -i swap
...
- 查询自己使用的是iptables还是nftables

```bash
$ ls -l /usr/sbin/iptables
lrwxrwxrwx. 1 root root 26  1月 18 00:33 /usr/sbin/iptables -> /etc/alternatives/iptables
$ ls -l /etc/alternatives/iptables
lrwxrwxrwx. 1 root root 22  1月 18 00:33 /etc/alternatives/iptables -> /usr/sbin/iptables-nft
```
...
192.168.200.116	k8s-node1
192.168.200.117 k8s-node2
192.168.200.118 k8s-node3
EOF
```

#### 1.4.3 配置SSH免密登录（仅中转节点）

为了方便文件的copy我们选择一个中转节点（随便一个节点，可以是集群中的也可以是非集群中的），配置好跟其他所有节点的免密登录。这里选择k8s-node1节点：
...
$ ssh-copy-id -i ~/.ssh/id_rsa.pub k8s-node3

# 验证（能免密执行就算成功）
$ ssh emon@k8s-node2 'echo OK'
$ ssh emon@k8s-node3 'echo OK'
```
