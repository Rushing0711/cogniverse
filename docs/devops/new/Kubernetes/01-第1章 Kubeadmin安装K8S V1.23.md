...
```bash
# nfs服务端
$ echo "/nfs/data 192.168.200.0/24(rw,sync,all_squash,anonuid=65534,anongid=65534,no_subtree_check)" > /etc/exports
# 查看配置
$ exportfs -v
/nfs/data       192.168.200.0/24(sync,wdelay,hide,no_subtree_check,sec=sys,rw,secure,root_squash,all_squash)
# 验证
$ exportfs
/nfs/data       192.168.200.0/24
```
...
# 配置生效 exportfs -r 是 NFS 服务器管理的关键命令，用于动态重载 /etc/exports 配置而不中断服务。
$ exportfs -r
```
...
```bash
$ mkdir -p /nfs/data && mount -t nfs 192.168.200.116:/nfs/data /nfs/data
```
...
```bash
# 在任何NFS节点创建目录，若不创建，在Pod的Events会报错： mounting 192.168.200.116:/nfs/data/nginx-pv failed, reason given by server: No such file or directory
$ mkdir -p /nfs/data/nginx-pv && echo 111222 > /nfs/data/nginx-pv/index.html
...
3. **/nfs/data:** NFS 服务器导出的共享数据目录的路径（exportfs）
      ...
      containers:
        - name: nfs-client-provisioner
          ...
          env:
            ...
            - name: NFS_PATH
              value: /nfs/data // [!code focus:1]
      volumes:
        - name: nfs-client-root
          nfs:
            server: 192.168.200.116 // [!code focus:1]
            path: /nfs/data // [!code focus:1]
```
