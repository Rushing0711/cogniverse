...
| get        | 下载一个release。可用子命令：all/hooks/manifest/notes/values<br />如：`helm get values devops -n kubesphere-devops-system` <br />查看默认+用户自定义的values值：`helm get values devops --all -n  kubesphere-devops-system` |
| history    | 获取release历史                                              |
| install    | 安装一个chart                                                |
| list       | 列出release                                                  |
...
| search     | 根据关键字搜索chart。可用子命令：hub/repo<br />如：`helm search repo bitnami/postgresql -l` |
