...
# 查看服务器支持啥方法
curl -i -X OPTIONS http://localhost:3001/song
```
...
# 强制使用某 DNS
$ dig @8.8.8.8 registry.cn-beijing.aliyuncs.com +short
```

### 8.4 查看端口是否开启

-  `bash`  内置 TCP（无需安装任何工具！）

```bash
# 测试 192.168.200.1:7890 是否开放
$ timeout 3 bash -c 'cat < /dev/null > /dev/tcp/192.168.200.1/7890' && echo "Port open" || echo "Port closed"
```

- curl（适合 HTTP 服务）

```bash
# 如果返回代理错误（如 407）或 Clash 的响应，说明端口通
$ curl -v http://192.168.200.1:7890
# 是否可访问外网？
$ curl -x http://192.168.200.1:7890 -I https://www.google.com
```

-  `nc`（netcat）—— 更强大

```bash
$ nc -vz -w 3 -n 192.168.200.1 7890
```

| 参数   | 作用                                        |
| ------ | ------------------------------------------- |
| `-w N` | 设置超时时间（秒），避免卡住（例如 `-w 3`） |
| `-u`   | 使用 UDP 协议（默认是 TCP）                 |
| `-n`   | 跳过 DNS 反向解析（加快速度，避免卡在 DNS） |
