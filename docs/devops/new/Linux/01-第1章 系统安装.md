...
```bash
sudo tee /usr/local/bin/verify-vm-local-repos.sh <<'EOF'
#!/bin/bash
set -e

echo "[$(date +'%T')] 🔍 Verifying HGFS, ISO, and DNF repos..."

# Quick checks
[[ -d /mnt/hgfs ]] || { echo "❌ HGFS not accessible"; exit 1; }
[[ -s /mnt/hgfs/Sharing/Rocky-9.5-aarch64-dvd.iso ]] || { echo "❌ ISO missing or empty"; exit 1; }
mountpoint -q /mnt/local-repo || { echo "❌ /mnt/local-repo not mounted"; exit 1; }
[[ -d /mnt/local-repo/BaseOS/repodata && -d /mnt/local-repo/AppStream/repodata ]] || { echo "❌ Repo structure broken"; exit 1; }

# Validate repos with a real package each
for r in Rocky9.5-BaseOS Rocky9.5-AppStream; do
    echo "[$(date +'%T')] → Validating $r"
    p=$(dnf --repo "$r" list available 2>/dev/null | awk 'NR>1 && /^[a-zA-Z]/ {print $1; exit}')
    [[ $p ]] || { echo "❌ No packages in $r"; exit 1; }
    sudo dnf download --repo "$r" --downloaddir=/tmp --assumeyes "$p" >/dev/null
    sudo rm -f "/tmp/$p"*.rpm 2>/dev/null
done

echo "[$(date +'%T')] ✅ 🎉 ALL CHECKS PASSED!"
EOF

sudo chmod +x /usr/local/bin/verify-vm-local-repos.sh
```

8. 虚拟机挂起恢复后hgfs无效（系统恢复后自动调用）

```bash
$ sudo tee /usr/lib/systemd/system-sleep/resume-hgfs.sh <<'EOF'
#!/bin/bash
# Restart HGFS and ISO mounts after VM resume

if [[ "$1" == "post" ]]; then
    systemctl restart hgfs-mount.service
    systemctl restart iso-mount.service
fi
EOF

sudo chmod +x /usr/lib/systemd/system-sleep/resume-hgfs.sh
```
