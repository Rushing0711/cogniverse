...
# 使用变量便于维护
HGFS_MOUNT="/mnt/hgfs"
LOCAL_REPO_MOUNT="/mnt/local-repo"
LOCAL_REPO_NAME="Rocky9.5"
ISO_NAME="Rocky-9.5-aarch64-dvd.iso"
ISO_PATH="$HGFS_MOUNT/$ISO_NAME"

# 创建挂载点
sudo mkdir -p "$HGFS_MOUNT" "$LOCAL_REPO_MOUNT"

# 1. HGFS automount（通用共享）
sudo tee /etc/systemd/system/mnt-hgfs.automount > /dev/null <<'EOF'
[Unit]
Description=Auto-mount HGFS
...
Options=allow_other,uid=1000,gid=1000,defaults
[Install]
WantedBy=multi-user.target
EOF

# 2. ISO automount（DNF 仓库）
sudo tee /etc/systemd/system/mnt-local-repo.automount > /dev/null <<'EOF'
[Unit]
Description=Auto-mount Rocky ISO
Before=local-fs.target
[Automount]
Where=/mnt/local-repo
TimeoutIdleSec=300
[Install]
WantedBy=multi-user.target
EOF

# 注意：此处使用不带引号的 EOF 以展开 $ISO_PATH
sudo tee /etc/systemd/system/mnt-local-repo.mount > /dev/null <<EOF
[Unit]
Description=Rocky 9.5 DVD Repo
After=mnt-hgfs.automount
Wants=mnt-hgfs.automount
[Mount]
What=$ISO_PATH
Where=/mnt/local-repo
Type=iso9660
...
sudo systemctl enable --now mnt-hgfs.automount mnt-local-repo.automount

# 4. 配置 DNF 仓库
sudo tee /etc/yum.repos.d/${LOCAL_REPO_NAME}-BaseOS.repo > /dev/null <<EOF
[${LOCAL_REPO_NAME}-BaseOS]
name=${LOCAL_REPO_NAME}-BaseOS
baseurl=file://$LOCAL_REPO_MOUNT/BaseOS
enabled=1
gpgcheck=0
EOF

sudo tee /etc/yum.repos.d/${LOCAL_REPO_NAME}-AppStream.repo > /dev/null <<EOF
[${LOCAL_REPO_NAME}-AppStream]
name=${LOCAL_REPO_NAME}-AppStream
baseurl=file://$LOCAL_REPO_MOUNT/AppStream
enabled=1
gpgcheck=0
EOF

# 5. 验证脚本（已修复硬编码问题并增强健壮性）
sudo tee /usr/local/bin/verify-hgfs-local-repo.sh > /dev/null <<'EOF'
#!/bin/bash
set -euo pipefail
...
log INFO "🔍 Verifying HGFS, ISO, and DNF repos..."

# 配置（与主部署脚本保持一致）
HGFS_PATH="/mnt/hgfs"
REPO_PATH="/mnt/local-repo"
ISO_PATH="$HGFS_PATH/Rocky-9.5-aarch64-dvd.iso"
REPO_NAME="Rocky9.5"
...
wait_for_mount() {
    ...
    for ((i=1; i<=timeout; i++)); do
        if mountpoint -q "$path" && [[ -e "$path" ]]; then
...
# Step 1: 触发 HGFS automount
if ! mountpoint -q "$HGFS_PATH"; then
    log INFO "Triggering HGFS automount..."
    timeout 5 ls "$HGFS_PATH" >/dev/null 2>&1 || true
fi

wait_for_mount "$HGFS_PATH" "HGFS" || exit 1
...
# Step 2: 触发 ISO automount
if ! mountpoint -q "$REPO_PATH"; then
    log INFO "Triggering ISO automount..."
    timeout 5 ls "$REPO_PATH" >/dev/null 2>&1 || true
fi

wait_for_mount "$REPO_PATH" "ISO repo" 5 || exit 1

# 检查仓库目录
for dir in BaseOS AppStream; do
    if [[ ! -d "$REPO_PATH/$dir/repodata" ]]; then
        log ERROR "Missing $dir/repodata in ISO"
        exit 1
    fi
done

# Step 3: 验证 DNF 仓库
log INFO "Validating DNF repositories..."
for repo in "${REPO_NAME}-BaseOS" "${REPO_NAME}-AppStream"; do
    log INFO "Checking $repo..."
    
    if ! dnf repolist enabled | grep -q "^$repo"; then
        log ERROR "Repository $repo not enabled"
        exit 1
    fi
    
    if ! timeout 10 dnf --repo "$repo" list available --quiet 2>/dev/null | head -5; then
        log ERROR "Failed to list packages from $repo"
        exit 1
    fi
done

# 验证可以下载包（更健壮地提取包名）
TEST_PKG=$(dnf --repo "${REPO_NAME}-BaseOS" list available --quiet 2>/dev/null | awk '/^[a-zA-Z]/ {print $1; exit}')
if [[ -n "$TEST_PKG" ]]; then
    log INFO "Testing download of $TEST_PKG..."
    if timeout 30 dnf download --repo "${REPO_NAME}-BaseOS" --downloaddir=/tmp --assumeyes "$TEST_PKG" >/dev/null 2>&1; then
        sudo rm -f "/tmp/$TEST_PKG"*.rpm 2>/dev/null || true
        log SUCCESS "Package download test passed"
    else
        log ERROR "Package download test failed"
        exit 1
    fi
else
    log WARN "No testable package found in BaseOS repo (skipping download test)"
fi

log SUCCESS "✅ All checks passed! System is ready."
EOF

sudo chmod +x /usr/local/bin/verify-hgfs-local-repo.sh

# 6. 快照准备脚本
sudo tee /usr/local/bin/snapshot-prep.sh > /dev/null <<'EOF'
#!/bin/bash
set -euo pipefail
...
echo_info "Starting snapshot preparation..."

# 清理包缓存
echo_info "Cleaning package cache..."
dnf clean all 2>/dev/null || yum clean all || true
rm -rf /var/cache/dnf/* /var/cache/yum/* 2>/dev/null || true

# 卸载挂载点
echo_info "Checking mount points..."
for mnt in /mnt/hgfs /mnt/local-repo; do
    if mountpoint -q "$mnt"; then
        echo_info "Unmounting $mnt..."
        if timeout 5 umount "$mnt"; then
            echo_ok "Unmounted $mnt"
        else
            echo_warn "Force unmounting $mnt (lazy)..."
            umount -l "$mnt" || true
        fi
    else
        echo_info "$mnt not mounted"
    fi
done

# 停止 automount 服务
echo_info "Stopping automount services..."
systemctl stop mnt-local-repo.automount 2>/dev/null || true
systemctl stop mnt-hgfs.automount 2>/dev/null || true

# 清理临时文件
echo_info "Cleaning temporary files..."
rm -f /tmp/*.rpm /tmp/dnf-* 2>/dev/null || true

echo ""
printf '%*s\n' "50" '' | tr ' ' '='
echo_ok "Snapshot preparation completed!"
echo ""
echo "Next steps:"
echo "1. Run: /usr/local/bin/verify-hgfs-local-repo.sh (after reboot)"
echo "2. For offline snapshot: sudo systemctl poweroff"
echo "3. For online snapshot: Ensure services are stopped as needed"
printf '%*s\n' "50" '' | tr ' ' '='
EOF

sudo chmod +x /usr/local/bin/snapshot-prep.sh
...
echo ""
echo "🔧 使用方法："
echo "1. 测试配置: /usr/local/bin/verify-hgfs-local-repo.sh"
echo "2. 快照前清理: /usr/local/bin/snapshot-prep.sh"
echo ""
echo "📌 注意：首次使用 DNF 时会自动挂载所需文件系统"
```
