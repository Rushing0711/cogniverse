...
[React19+Hooks+TS+zustand+源码+推荐](https://www.bilibili.com/video/BV1mcpPeMETt/?spm_id_from=333.337.search-card.all.click&vd_source=b850b3a29a70c8eb888ce7dff776a5d1)
...
[cra的template用法](https://create-react-app.dev/docs/getting-started/#selecting-a-template)

[cra支持的template列表](https://www.npmjs.com/search?q=cra-template-*)
...
- 比如，创建支持TypeScript的项目

```bash
$ create-react-app react-03-rtk-ts --template typescript
```

## 1.5 使用Vite平替create-react-app

[Vite替换CRA](https://www.bilibili.com/video/BV18r421t7iw/?spm_id_from=333.1387.homepage.video_card.click&vd_source=b850b3a29a70c8eb888ce7dff776a5d1)

```bash
$ pnpm create vite react-12-zustand
```
...
[官方网站](https://zustand-demo.pmnd.rs/)

[为什么 Zustand 的 Typescript 实现如此丑陋](https://blog.axlight.com/posts/why-zustand-typescript-implementation-is-so-ugly/)
...
# 9 TanStack Query（曾经的React Query)
...
## 9.1 React Query v5 - `useQuery` 返回对象属性详解

| 类别         | 属性                  | 类型                                  | 简短说明                                                     |
| :----------- | :-------------------- | :------------------------------------ | :----------------------------------------------------------- |
| **核心状态** | `status`              | `'pending'` | `'error'` | `'success'` | 查询的**最终状态**（互斥）                                   |
|              | `isPending`           | `boolean`                             | `status === 'pending'` 的派生布尔值                          |
|              | `isError`             | `boolean`                             | `status === 'error'` 的派生布尔值                            |
|              | `isSuccess`           | `boolean`                             | `status === 'success'` 的派生布尔值                          |
| **数据**     | `data`                | `TData` | `undefined`                 | 查询成功返回的数据。默认为 `undefined`                       |
|              | `dataUpdatedAt`       | `number`                              | 数据最后一次成功更新的时间戳（毫秒）                         |
| **错误**     | `error`               | `TError` | `null`                     | 查询失败时抛出的错误对象。默认为 `null`                      |
|              | `errorUpdatedAt`      | `number`                              | 错误最后一次发生的时间戳（毫秒）                             |
|              | `failureCount`        | `number`                              | 当前连续的失败次数，成功时重置为 0                           |
|              | `failureReason`       | `TError` | `null`                     | 查询重试失败的原因，成功时重置为 `null`                      |
|              | `isLoadingError`      | `boolean`                             | **首次加载**失败时为 `true`                                  |
|              | `isRefetchError`      | `boolean`                             | **后台重新获取**失败时为 `true`                              |
| **获取状态** | `fetchStatus`         | `'fetching'` | `'paused'` | `'idle'`  | 查询**过程的当前状态**（可并行）                             |
|              | `isFetching`          | `boolean`                             | **任何**获取请求正在进行中（`fetchStatus === 'fetching'`）   |
|              | `isPaused`            | `boolean`                             | 查询想获取但被暂停了（如离线状态）                           |
|              | `isLoading`           | `boolean`                             | **首次加载且无数据**时的获取状态（`isFetching && isPending`） |
|              | `isRefetching`        | `boolean`                             | **已有数据时的后台刷新**（`isFetching && !isPending`）       |
| **元信息**   | `isStale`             | `boolean`                             | 数据已**过期**（超过 `staleTime`），需要重新获取             |
|              | `isFetched`           | `boolean`                             | 查询**至少尝试过获取一次**（无论成功与否）                   |
|              | `isFetchedAfterMount` | `boolean`                             | 在**组件挂载后**至少获取过一次                               |
|              | `isPlaceholderData`   | `boolean`                             | 当前显示的数据是预先设置的占位数据                           |
| **方法**     | `refetch`             | `function`                            | **手动触发**重新获取数据的函数                               |
|              | `promise`*            | `Promise<TData>`                      | 一个稳定的 Promise（需启用实验性功能）                       |

**核心逻辑关系（帮助你记忆）**

1. **`status` vs `fetchStatus`**:
   - `status`：回答“**我有什么？**” (data, error, or nothing)
   - `fetchStatus`：回答“**我正在做什么？**” (fetching, idle, or paused)
2. **`isLoading` vs `isFetching`**:
   - `isLoading = isFetching && isPending`
   - `isLoading` **是 `isFetching` 的子集**，特指“第一次加载，还没有任何数据”的那个加载状态。
3. **`isRefetching`**:
...
| 场景                         | 推荐使用的属性组合                             |
| :--------------------------- | :--------------------------------------------- |
| **初始加载（无缓存）**       | `isLoading` 或 `(isPending && !data)`          |
| **显示加载骨架屏**           | `isLoading`                                    |
| **显示成功获取的数据**       | `isSuccess && data`                            |
| **显示错误信息**             | `isError && error`                             |
| **后台刷新时显示小加载图标** | `isFetching && !isLoading` (即 `isRefetching`) |
| **判断数据是否为最新**       | `!isStale`                                     |
| **手动重新获取数据**         | `refetch()`                                    |

最常使用的就是 `data`, `isLoading`, `isError`, `error`, `isFetching` 和 `refetch` 这几个核心属性。

## 9.2 useMutation 返回结果对象属性/方法说明

| 属性/方法           | 类型                    | 说明                                     |
| :------------------ | :---------------------- | :--------------------------------------- |
| **`mutate`**        | `function`              | 触发 mutation 的函数，传入变量和可选回调 |
| **`mutateAsync`**   | `function`              | 类似 mutate，但返回 Promise，可 await    |
| **`status`**        | `MutationStatus`        | 当前状态：idle、pending、success、error  |
| **`isIdle`**        | `boolean`               | 是否为初始空闲状态                       |
| **`isPending`**     | `boolean`               | 是否正在执行中                           |
| **`isSuccess`**     | `boolean`               | 是否成功完成                             |
| **`isError`**       | `boolean`               | 是否执行失败                             |
| **`isPaused`**      | `boolean`               | 是否被暂停（网络模式相关）               |
| **`data`**          | `undefined｜unknown`    | 最后一次成功返回的数据                   |
| **`error`**         | `null｜TError`          | 错误信息（执行失败时）                   |
| **`reset`**         | `function`              | 重置 mutation 到初始状态                 |
| **`failureCount`**  | `number`                | 失败次数，成功时重置为 0                 |
| **`failureReason`** | `null｜TError`          | 重试失败的原因，成功时重置为 null        |
| **`submittedAt`**   | `number`                | mutation 提交的时间戳                    |
| **`variables`**     | `undefined｜TVariables` | 传递给 mutation 函数的变量               |
...
| **`onError`**   | `(error, variables, context)`       | mutation 失败时触发                 |
| **`onSettled`** | `(data, error, variables, context)` | mutation 完成时触发（无论成功失败） |
