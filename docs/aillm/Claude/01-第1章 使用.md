...
| **gstack** | 82k+ | 虚拟 23 人工程团队 + Power Tools | 48 个 | Solo 冲刺产品 MVP | `git clone` 到 `~/.claude/skills/gstack` |
| **mattpocock** | 60k+ | 慢下来，先想清楚再动手 | 12 个 | TypeScript 开发 | `npx skills@latest add mattpocock/skills` |
| **OpenSpec** | 18k+ | Spec 驱动开发（SDD） | 1 套 CLI + 多命令 | 需求到交付的完整闭环 | `npm install -g @fission-ai/openspec@latest` |
...
**💡 按星筛选安装**：五星（★★★★★）是日常必装——`test-driven-development`、`writing-plans`、`systematic-debugging`、`using-superpowers`。四星补充 `brainstorming`、`executing-plans`、`requesting-code-review`、`verification-before-completion`。三星以下按场景按需加装。只需 4-8 个核心 Skill，token 开销可控。
...
OpenSpec 是 Fission AI 出品的 Spec-Driven Development（SDD）框架，18k+ Stars。核心理念：**先对齐 spec，再写代码**——人类和 AI 在写任何代码之前，就通过结构化 spec 文件达成共识。

**vs. Superpowers vs. gstack 的定位差异**：

| 维度 | Superpowers | gstack | OpenSpec |
|------|------------|--------|----------|
| 核心理念 | 工程纪律约束 AI | 角色扮演模拟团队 | Spec 驱动开发 |
| 工作方式 | 自动触发流程 | 手动 `/` 命令调用 | Slash 命令 + 文件系统 spec |
| 产物 | 代码 + 测试 | 代码 + 设计文档 | proposal.md + specs/ + design.md + tasks.md |
| 适合 | 日常开发的完整闭环 | 0→1 快速出 MVP | 需求明确、需要多人协作的功能 |
| 哲学 | fluid → rigid（约束 AI） | think → plan → build → review → ship | fluid not rigid（迭代式） |
...
| `/opsx:continue` | Custom | ★★★ | 按依赖图逐个创建产物（proposal → specs → design → tasks），每步审查 |
...
| `/opsx:onboard` | Custom | ★★ | 交互式新手引导，用你的实际代码库走完整流程（约 15-30 分钟） |

**💡 按星筛选使用**：日常只需 Core 模式的 5 个命令。★★★ 以下 Custom 命令在复杂功能或团队协作时按需启用——`openspec config profile` → 选 custom → 勾选需要的 workflow → `openspec update`。
...
**Delta Spec 机制**——OpenSpec 的核心概念。Delta spec 用三个区段描述变更：
...
归档时：ADDED 追加到主 spec → MODIFIED 替换对应条目 → REMOVED 从主 spec 删除。非破坏性合并，保留完整审计历史。

**vs. 竞争对手**：

- **vs. GitHub Spec Kit**：Spec Kit 更重（Python 依赖、严格的阶段门控），OpenSpec 更轻量灵活
- **vs. Kiro（AWS）**：Kiro 锁死在 IDE 内且仅支持 Claude 模型；OpenSpec 支持 25+ 工具（Claude Code、Codex、Cursor 等）
- **vs. 不用 spec**：没有 spec 的 AI 编码意味着模糊的 prompt + 不可预测的输出。OpenSpec 用最小仪式感换可预测性
...
**何时用 OpenSpec vs. Superpowers vs. gstack**：

- 需求模糊、需要探索 → **OpenSpec** 的 `/opsx:explore` 先摸清代码库和方案空间
- 需求模糊、需要被追问 → **Superpowers** 的 brainstorming 苏格拉底式追问
- 有明确需求、需要多人或跨会话协作 → **OpenSpec** 先写 spec 再实施
- 一个人冲刺 MVP、需要方向感 → **gstack** 的 CEO/QA 角色驱动

**与 Superpowers 互补的最佳姿势**：OpenSpec 做 spec 层（`explore` → `propose` → spec files），Superpowers 做执行层（`test-driven-development` + `requesting-code-review` 实现 tasks.md 中的每个任务）。两者不相冲突——OpenSpec 管"做什么"，Superpowers 管"怎么做"。

### 10.5 gstack 实战 — 一人成军

gstack 是 Y Combinator CEO Garry Tan 的开源作品，82k+ Stars，MIT 许可。它把 Claude Code 变成一个虚拟的 23 人工程团队——从 CEO 到 QA，每个人都能独立履职。
...
| **测试** | `/qa-only` | ★★★ | QA Reporter | 纯报告模式，不改代码，产出 bug 报告 + 复现步骤 |
...
**💡 按星筛选安装**：五星日常四件套 — `/review`、`/ship`、`/qa`、`/investigate`，正好也是 gstack 作者推荐的日常维护命令。四星补充 `/office-hours`、`/plan-ceo-review`、`/plan-eng-review`、`/codex`、`/retro`，覆盖思考和规划场景。三星以下按项目阶段按需调用（设计阶段用 design 系列，发布部署用 `/land-and-deploy` 等）。
...
| 部署 | `/setup-deploy`, `/setup-browser-cookies` | ★★★ | 部署配置 + 浏览器 Cookie 导入 |
| 知识 | `/learn`, `/context-save`, `/context-restore` | ★★★★ | 跨会话学习 + 上下文保存/恢复 |
...
| 同步 | `/setup-gbrain`, `/sync-gbrain`, `/gstack-upgrade` | ★★ | GBrain 部署/同步 + gstack 自更新 |
...
Matt Pocock（Total TypeScript 作者）开源的 Skills 套装，68k+ Stars。核心理念：**让 AI 慢下来，先想清楚再动手**。12 个活跃 Skill 覆盖从方案推敲、TDD 编码、调试诊断到 Issue 管理的完整闭环。
...
# 按需安装单个 Skill
npx skills@latest add mattpocock/skills/grill-me       # 方案追问
npx skills@latest add mattpocock/skills/tdd            # TDD 红绿重构
npx skills@latest add mattpocock/skills/diagnose        # 结构化调试
npx skills@latest add mattpocock/skills/git-guardrails-claude-code  # Git 危险命令拦截
```
...
| # | Skill / 命令 | 常用度 | 类别 | 触发场景 | 实际效果 |
|---|-------------|--------|------|----------|----------|
| 1 | `/grill-me` | ★★★★★ | 思考 | "我打算做 X 功能" | 像技术面试官连续追问 16-50 个问题，直到每个决策分支都想清楚 |
| 2 | `grill-with-docs` | ★★ | 思考 | grill-me 的同时需要记录 | 追问 + 自动更新 `CONTEXT.md` 和 ADR 决策文档 |
| 3 | `tdd` | ★★★★★ | 编码 | "实现 X 功能" | 严格 Red→Green→Refactor 循环，强制先写失败测试再写实现 |
| 4 | `diagnose` | ★★★★★ | 调试 | "修这个 bug" | 6 阶段诊断：复现→最小化→假设→插桩→修复→回归测试 |
| 5 | `to-prd` | ★★★ | 文档 | 方案讨论完毕 | 将当前对话内容合成 PRD，提交为 GitHub Issue |
| 6 | `to-issues` | ★★★ | 文档 | PRD/方案已就绪 | 将方案拆成独立垂直切片的 GitHub Issues |
| 7 | `triage` | ★★★ | 管理 | Issue 列表混乱 | 按状态机分诊：`needs-triage`→`needs-info`/`ready-for-agent`/`ready-for-human`/`wontfix` |
| 8 | `zoom-out` | ★★★★ | 探索 | "这个模块我不熟悉" | 给出代码区域的高层地图，而非逐行解释 |
| 9 | `improve-codebase-architecture` | ★★ | 重构 | 代码腐化需要清理 | 基于 `CONTEXT.md` 领域语言和 ADR 决策发现深层改进机会 |
| 10 | `caveman` | ★★★★ | 效率 | Token 消耗太高 | 超压缩通信模式，砍掉 ~75% token 同时保持技术准确性 |
| 11 | `prototype` | ★★★ | 探索 | "这个方案可行吗？" | 构建可丢弃原型来暴露设计问题：终端版（逻辑验证）或 UI 多变体 |
| 12 | `setup-matt-pocock-skills` | ★ | 配置 | 首次安装后 | 搭建项目级配置（Issue 跟踪器、分诊标签、文档结构），每个仓库运行一次 |

**💡 按星筛选安装**：五星日常三件套 — `/grill-me`、`tdd`、`diagnose`，覆盖方案推敲→编码→调试全流程。四星补充 `zoom-out`（探索代码）、`caveman`（省 token）。三星的 `to-prd`/`to-issues`/`triage` 在多人协作时有价值，`prototype` 用于方案验证。

**额外实用 Skill（不在 12 个核心中，但仓库里有）**：

| Skill | 常用度 | 用途 |
|-------|--------|------|
| `git-guardrails-claude-code` | ★★★★ | 在 git hook 层拦截危险命令（force push、hard reset 等） |
| `handoff` | ★★★ | 将当前会话压缩成交接文档，给另一位 Agent 继续 |
| `write-a-skill` | ★★ | 元技能，用 TDD 方式创建新的 Skill |
| `migrate-to-shoehorn` | ★ | 将 `as` 断言迁移到 `@total-typescript/shoehorn` |
| `scaffold-exercises` | ★ | 生成练习目录结构（教学场景） |

**`/grill-me` 的核心价值**：这是整个套装最出名的 Skill。Matt 本人曾被自己的 `grill-me` 连续追问近 50 个问题才通过。适合写重逻辑代码前逼自己理清思路。问的问题包括："这个组件需要什么 props？""状态从哪里来？服务端还是客户端？""loading 态长什么样？""如果 API 超时怎么办？""可访问性怎么处理？"

**与其他套装的定位差异**：

| | mattpocock | Superpowers | gstack |
|------|-----------|------------|--------|
| 核心哲学 | 慢下来先想清楚 | 工程纪律约束 AI | 角色扮演模拟团队 |
| 触发方式 | 手动 `/` 命令为主 | 自动触发为主 | 手动 `/` 命令 |
| 强项 | 方案推敲 + TDD 编码 | 全流程自动化 | 0→1 快速出 MVP |
| 适合理念 | "多问几句不费电" | "把 AI 当初级工程师管" | "一个人 + AI 团队" |
...
**使用建议**：
- `/grill-me` 配合 `gg`（GitHub Copilot 的 grill）或 Superpowers 的 `brainstorming` 使用效果更好
- `tdd` + `diagnose` 是日常编码最常用的两个
- `caveman` 在 token 紧张时非常实用，能省 75% 的 token
...
GSD（Get Shit Done）由 TÂCHES 团队出品，35k+ Stars。核心理念：**不要让上下文腐化**——每个开发阶段使用独立上下文窗口，解决长会话"AI 忘记前面约定"的通病。
...
安装后重启 CC，GSD 的 `/gsd:*` 命令即可用。支持 Claude Code、Copilot、Gemini CLI 等 15 种运行时。
...
**💡 按星筛选安装**：GSD 的核心是 ★★★★★ 的 `/gsd-execute-phase`，日常最频繁使用。四星命令 `/gsd-discuss-phase`、`/gsd-plan-phase`、`/gsd-ship` 覆盖讨论→规划→发布，完整走 6 阶段时全用上。三星的 `/gsd-verify-work` 和 `/gsd-complete-milestone` 在正式发布场景使用。
...
| `/gsd-map-codebase` | ★★★ | 在初始化前分析现有代码库结构 |
...
| | GSD | OpenSpec | Superpowers |
|------|-----|----------|------------|
| 核心哲学 | 阶段隔离、上下文工程 | Spec 驱动、人类对齐 | 工程纪律约束 AI |
| 上下文管理 | 每个阶段用独立上下文窗口 | 同一会话内流转 | 同一会话内自动触发 |
| 适合场景 | 需求模糊、需要多轮迭代 | 需求明确、需要多人协作 | 日常开发的完整闭环 |
| 学习成本 | 中（需理解 6 阶段概念） | 低（2-3 个核心命令） | 低（自动触发为主） |
...
- 与 Superpowers 互补：GSD 管"分阶段隔离上下文"，Superpowers 管"每个阶段内的工程纪律"
...
**planning-with-files** — 最轻量的持久记忆方案，13K+ Stars：

```bash
npx skills add OthmanAdi/planning-with-files
```

用 `task_plan.md`、`findings.md`、`progress.md` 作工作记忆，CC 自动读取和更新。轻量无侵入，适合不想装重型套装、只需要防止上下文丢失的场景。
...
| **多人协作功能** | OpenSpec + Superpowers | OpenSpec 写 spec 对齐需求，Superpowers 做 TDD 实现和审查 |
...
### 10.12 按星自由组合：最小 token 开销的安装方案

与其全套装满，不如"跨套装择优"——只挑每套中五星和四星的技能，组成自己的轻量工具箱。以下是三种按星组合方案：

**方案 A：极简日常（≈8 个 Skill，适合个人项目）**

```bash
# Superpowers 五星核心（4 个）
npx skills add obra/superpowers -g -a claude-code \
  --skill test-driven-development \
  --skill writing-plans \
  --skill systematic-debugging \
  --skill using-superpowers

# mattpocock 五星三件套（3 个）
...
# gstack 五星日常四件套（如果装 gstack）
git clone --single-branch --depth 1 https://github.com/garrytan/gstack.git \
  ~/.claude/skills/gstack && cd ~/.claude/skills/gstack && ./setup
# 日常只用 /review + /ship + /qa + /investigate
```

**方案 C：0→1 新项目（GSD 打底 + Superpowers 五星）**

```bash
# GSD（一套完整 6 阶段流程）
npx get-shit-done-cc@latest

# Superpowers 五星核心覆盖执行质量
npx skills add obra/superpowers -g -a claude-code \
  --skill test-driven-development \
  --skill systematic-debugging \
  --skill using-superpowers
...
| 套装 | 全量 Skill 数 | 安装后常驻上下文占比（估算） | 按星挑选后 |
|------|-------------|--------------------------|----------|
| Superpowers | 14 个 | ~15-20% | 只装 ★★★★+ 约 8 个，占 ~8-10% |
| gstack | 23 角色 + 24 工具 = 47 个 | ~30-40% | 日常只用 4 个五星命令，占 ~5-8% |
| mattpocock | 12 核心 + 5 额外 | ~10-15% | 只装 ★★★★+ 约 6 个，占 ~5-7% |
| GSD | 全套 6 阶段 + 辅助 | ~15-20% | 全装不拆分，粒度合理 |
| OpenSpec | 9 个命令 | ~3-5% | 本身就轻量，全装无压力 |

**总结**：跨套装"择优混搭"后，日常维护只需 8-15 个 Skill，token 开销控制在 10-15% 以内。关键原则：五星必装，四星按场景补充，三星以下等需要时再加装。
