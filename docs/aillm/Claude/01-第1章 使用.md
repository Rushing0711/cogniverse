...
| 命令 | 用途 |
|------|------|
| `/opsx:propose <idea>` | 生成提案（proposal + specs + design + tasks） |
| `/opsx:apply` | 按 tasks.md 逐项实现 |
| `/opsx:archive` | 归档变更，更新主 specs |
| `/opsx:new` | 新工作流入口（扩展模式） |
| `/opsx:continue` | 继续未完成的任务 |
| `/opsx:ff` | 快速推进（fast-forward） |
| `/opsx:verify` | 验证实现是否匹配 spec |
| `/opsx:bulk-archive` | 批量归档已完成变更 |
| `/opsx:onboard` | 初始化项目 OpenSpec 配置 |
...
- 需求模糊、需要探索 → **Superpowers** 的 brainstorming 追问
- 有明确需求、需要多人或跨会话协作 → **OpenSpec** 先写 spec 再实施
- 一个人冲刺 MVP、需要方向感 → **gstack** 的 CEO/QA 角色驱动

**与 Superpowers 互补的最佳姿势**：OpenSpec 做 spec 层（`propose` → spec files），Superpowers 做执行层（`test-driven-development` + `requesting-code-review` 实现 tasks.md 中的每个任务）。两者不相冲突——OpenSpec 管"做什么"，Superpowers 管"怎么做"。
