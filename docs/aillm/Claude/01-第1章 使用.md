...
除了选择模式，还可以精细控制哪些操作免确认。在 `~/.claude/settings.json`（全局）或 `.claude/settings.json`（项目级）中配置：
...
| `"Edit"` | 允许所有文件编辑 |
| `"Write"` | 允许创建新文件 |
| `"Bash(npm run *)"` | 允许 `npm run xxx`，`*` 通配符 |
| `"Bash(git commit:*)"` | 允许所有 git commit |
| `"WebFetch"` | 允许抓取 URL |
| `"WebSearch"` | 允许网络搜索 |
...
**更细粒度控制**：用 `deny` 数组明确禁止某些操作，或用 `/permissions` 在会话中动态调整。
...
在会话中按 `#` 键，输入指令，Claude 自动写入对应的 CLAUDE.md。日常编码时顺手用 `#` 记录命令和规范。
