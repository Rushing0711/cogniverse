...
```
SessionStart
├── InstructionsLoaded ← CLAUDE.md / rules 加载时触发（可多次）
│
UserPromptSubmit ← 用户输入处理开始
├── UserPromptExpansion ← 若输入是斜杠命令，展开后触发
│
  ┌── 代理循环（每轮 LLM 调用可能产生多个工具调用）──┐
  │                                                  │
  │  PreToolUse ← 工具执行前                          │
  │  ├── Elicitation ← MCP 弹窗（可选）               │
  │  ├── ElicitationResult ← 用户响应 MCP 弹窗（可选） │
  │  ├── PermissionRequest ← 需要权限确认时          │
  │  │   └── PermissionDenied ← auto 模式拒绝时      │
  │  ├── PostToolUse ← 执行成功                      │
  │  ├── PostToolUseFailure ← 执行失败               │
  │  └── PostToolBatch ← 并行工具全部完成             │
  │                                                  │
  │  SubagentStart → SubagentStop ← 子代理嵌套       │
  │  TaskCreated → TaskCompleted ← 任务管理          │
  └──────────────────────────────────────────────────┘
  （循环直到 Claude 决定停止）
│
Stop ← 正常完成
├── StopFailure ← API 错误时替代 Stop
│
PreCompact ← 上下文快满时
├── PostCompact
│
SessionEnd ← 会话终止，最终清理
```

**独立事件**（不属于上述主链，条件触发）：

```
WorktreeCreate → WorktreeRemove   ← worktree 隔离模式
Notification                      ← 系统通知
ConfigChange                      ← 配置热变更
CwdChanged                        ← cd 切换目录
FileChanged                       ← watchPaths 文件变更
