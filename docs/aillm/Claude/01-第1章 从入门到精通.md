...
| `CLAUDE_CODE_ATTRIBUTION_HEADER` | 自定义请求中的 attribution 头，用于标识客户端版本。接入第三方模型时可能需要设为空或自定义值以避免兼容问题 | `""` 或自定义字符串 |

> **注意**：`ANTHROPIC_AUTH_TOKEN` 和 `ANTHROPIC_API_KEY` 同时设置会冲突，二选一。接入第三方非 Claude 模型时，如果遇到 400 错误或 beta header 相关报错，尝试设置 `CLAUDE_CODE_ATTRIBUTION_HEADER=""` 和 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1`。
