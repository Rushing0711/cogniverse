...
Hooks 有完整的生命周期，按触发阶段可分为四个层级：

| 层级 | 范围 | 关键事件 | 说明 |
|------|------|----------|------|
| **会话级** | 整个 CC 会话 | `SessionStart` → `SessionEnd` | 包裹一次完整会话，启动时加载环境，结束时清理 |
| **Turn 级** | 一次用户交互 | `UserPromptSubmit` → (`PreToolUse` → `PostToolUse`) × N → `Stop` | 每轮对话的核心循环：提交 → 工具调用 → 完成 |
| **子代理级** | 一次子代理任务 | `SubagentStart` → `SubagentStop` | 嵌套在 Turn 内，独立上下文执行 |
| **上下文管理** | 上下文压缩 | `PreCompact` → `PostCompact` | 释放 token 时的保存与恢复 |

此外还有**事件驱动**的事件（`ConfigChange`、`CwdChanged`、`FileChanged`）和**MCP 交互**事件（`Elicitation`、`ElicitationResult`），它们不固定在某一个层级，在满足条件时触发。

![Hook生命周期](images/6be7323542e11148c4738401434aa70b.png)
...
| 事件 | 触发时机 | 典型用途 |
|------|----------|----------|
| `PreToolUse` | 工具执行**前** | 拦截危险命令、阻止编辑敏感文件、修改工具输入 |
| `PostToolUse` | 工具执行成功**后** | 编辑文件后自动 lint/格式化、记录操作日志 |
| `PostToolUseFailure` | 工具执行失败**后** | 捕获错误详情、失败重试或降级处理 |
| `UserPromptSubmit` | 用户提交提示词时 | 提示词预处理、注入动态上下文、敏感信息过滤 |
| `PermissionRequest` | 弹出权限确认框时 | 自定义权限逻辑、自动拒绝特定操作 |
| `PermissionDenied` | auto 模式分类器拒绝工具调用时 | 返回 `{retry: true}` 让分类器重新评估 |
| `Stop` | Claude 完成响应时 | 清理临时文件、发送会话摘要、强制继续执行 |
| `StopFailure` | 因 API 错误（限流/认证失败等）导致 turn 结束时 | 错误恢复、重试逻辑 |
| `SessionStart` | 会话启动或恢复时 | 环境检查、注入 git 状态、加载 TODO 列表 |
| `SessionEnd` | 会话终止时 | 清理临时文件、保存状态、归档日志 |
| `PreCompact` | `/compact` 压缩**前** | 压缩前保存关键上下文、追加需要保留的信息 |
| `PostCompact` | `/compact` 压缩**后** | 验证压缩结果、记录压缩日志 |
| `SubagentStart` | 子代理任务开始时 | 注入子代理上下文、环境准备 |
| `SubagentStop` | 子代理任务结束时 | 子代理结果后处理、强制继续执行 |
| `Notification` | Claude 收到通知时 | 自定义通知响应逻辑、条件过滤 |
| `InstructionsLoaded` | CLAUDE.md / `.claude/rules/` 等指令文件加载后 | 调试：记录哪些指令被加载了 |
| `ConfigChange` | 会话中配置文件变更时 | 热重载配置、自动应用新设置 |
| `CwdChanged` | 工作目录变更时（如 direnv 触发） | 环境变量同步、路径检查 |
| `FileChanged` | 监听的文件变更时 | 热重载、自动构建触发（需 matcher 指定文件名） |
| `WorktreeCreate` | 子代理隔离模式创建 worktree 时 | 自定义 worktree 创建逻辑 |
| `WorktreeRemove` | 子代理隔离模式删除 worktree 时 | 自定义 worktree 清理逻辑 |
| `Setup` | `/setup`、`--init` 或维护模式启动时 | 项目初始化、环境检查、依赖安装 |
| `Elicitation` | MCP 服务器在工具调用中请求用户输入时 | 自定义 MCP 交互流程 |
| `ElicitationResult` | 用户响应 MCP elicitation 后 | 处理用户输入、发送前验证 |
